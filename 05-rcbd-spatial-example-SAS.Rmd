...
These data are from a winter wheat variety trial in Alliance, Nebraska [@stroup1994] and are commonly used to demonstrate spatial adjustment in linear modeling. The data represent yields of 56 varieties originally layed out in a randomized complete block design (RCBD). The local topography combined with winter kill, however, induced spatial variability that did not correspond to the RCBD design [@stroup2013] which produced biased estimates in a standard unadjusted analysis. <br><br>

The code below reads the data from a CSV file where missing values are denoted as a single period in the data file. These values are not actually missing, but represent blank plots in the original lay out of the study. They are removed in this data step before proceeding to analyses. Also note that row and column indices are multiplied by constants to convert them to the meter dimensions of the plots.<br><br>

The first six observations are displayed. The data contains variables for variety (entry), replication (rep), yield, and column and row identifiers.<br><br> Last, a map of the 4 replications (blocks) from the original experimental design is shown in gray shading.

```{r eval=FALSE, include=FALSE}
# here's an option for getting the data, it's currently not being shown or run
library(agridat); data("stroup.nin"); library(dplyr)

Nin.na <- stroup.nin %>% mutate(rep = as.character(rep)) %>% 
  tidyr::replace_na(list(rep = ".", yield = "."))

#write.csv(Nin.na, "data/Stroup.NIN.wheat.csv") # this will overwrite the existing file
writexl::write_xlsx(Nin, "data/Stroup.NIN.wheat.xlsx")
rm(Nin, Nin.na)
```


```{r code1, engine="sashtml", collectcode=TRUE, engine.path=saspath, include=TRUE, engine.opts=sasopts, comment=""}
# let's see if we can get to work, then we can work on 
data alliance;
    infile 'data\Stroup.NIN.wheat.csv' firstobs=2 delimiter=',';
    #infile 'C:\Users\bprice\Dropbox\Spatial\Stroup.NIN.wheat.csv' firstobs=2 delimiter=',';
...
In the heat map, there is a notable region where yield dips in the north west corner of the study area. This area runs across the two top most blocks and is positioned towards their ends, making those blocks non-homogeneous. As Stroup (2013) notes, this is due to a hilly area with low snow cover and high exposure to low winter temperatures. The box plots demonstrate this pattern across blocks, columns, and rows as well. From these initial graphics, it is clear there is discernible spatial variability present.<br><br>
...
In Section 3, several variogram models were described. We can use **Proc Variogram** to fit and compare any number of these models. In the code below, the Gaussian, Exponential, Power, and Spherical models are fit using the **model** statement.<br><br>
