...
There is a rich field of research on using localised splines to model field heterogeneity. These models are complex and hence not described.  
...
First, create a spatial object by adding spatial coordinates to an ordinary data frame. I like to use the actual units of the plots since they are exaggerated rectangles (meters in this example).
...
In this field trial, there is evidence spatial correlation as a function of distance, but there is not evidence that spatial auto-correlation is impacted by the direction.
...
# Applying Spatial Analytical Techniques {#ch4}
...
Below is the R code for RCBD analysis of the NIN data set loaded in \@ref(ch:ch3) that was found to have strong spatial correlation:
...
At this time, there are not suitable options for running spatial regression of variety trial data. Existing options in **spatialreg**, **spANOVA**, and **spaMM** are extraordinarily difficult to use, lack sufficient options for typical experimental design in variety trials and/or do not provide the desired output. 
...
As a result, using the coefficient of variation is not a good metric for evaluating the quality of spatial models. 
...
CV
```
I would further argue that in general, the coefficient of variation is not an effective metric for evaluating confidence in a variety trials. 
...
Gather together predictions generated in \@ref(ch:ch4):
```{r}
...
```{r SE-box-fig, echo=FALSE, fig.cap='Differences in Variety Standard Error', out.width='80%', fig.asp=0.65, fig.align='center'}
...
```{r ls_panel-fig, echo=FALSE, fig.align='center', fig.asp=1, fig.cap='Correlations in Variety Means', message=FALSE, warning=FALSE, out.width='90%'}
library(psych)
