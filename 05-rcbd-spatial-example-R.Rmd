...
The estimates for $\rho$ are determined via a grid search. The default grid searches the entire space, ([-1, 1] for rows and columns) We can narrow down the grid search to the area most impactful based on log liklihood. 
```{r}
...
```{r}
nlme_mods <- list(nin_lme, nin_trend, nin_exp, nin_gaus, nin_sph, nin_pow, nin_matern)

names(nlme_mods) <- c(c("LMM", "row-col_trend", "exponential", 
...
lev <- c("lme", "trend", "exp", "gaus", "mat", "sph", "spline") #, "ar1ar1")

#melt(preds_df, id.vars = "gen", variable.name = "model", value.name = "emmeans") %>% 
pivot_wider(preds_df, id_cols = gen, names_from = model, values_from = emmeans) %>% 
