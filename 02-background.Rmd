...
The statistical model for that experiment. (let's pretend $X_i$ are different fertilizer treatments applied randomly - this is not ideal, but it's a nice small example)
...
Sometimes called a "lag" model, the AR1 model assumes that variance can be modelled as exponential function based unit distance in a single direction. 

The AR1 structure across 2 rows is modelled as thus: 
...
And similarily the AR1 structure across 4 ranges is modelled as thus: 
...
The moving average model is quite similar to auto-regressive model. Auto-regressive models a variables's spatial dependence on nearby experimental units. 
...
Experiment wide-trends should be modelled with directional trend models. These are comparatively simple models:
