...
```{r SAS1, engine="sashtml", collectcode=TRUE, engine.path=saspath, include=TRUE, engine.opts=sasopts, comment=""}
proc format;
  invalue has_NA
  'NA' = .;
;
 ...
 data alliance;
    infile '.\data\stroup_nin.csv' firstobs=2 delimiter=',';
...
```{r SAS2, engine="sashtml",  engine.path=saspath, engine.opts=sasopts, include=TRUE, comment=""}
ODS html GPATH = ".\img\" ;
...
```{r SAS3, engine="sashtml", collectcode=TRUE, engine.path=saspath, include=TRUE, engine.opts=sasopts, comment=""}
...
```{r SAS4, engine="sashtml",  engine.path=saspath, engine.opts=sasopts, include=TRUE, comment=""}
...
```{r SAS5, engine="sashtml",  engine.path=saspath, engine.opts=sasopts, include=TRUE, comment=""}
ODS html GPATH = ".\img\" ;
...
```{r SAS6, engine="sashtml", engine.path=saspath, include=TRUE, engine.opts=sasopts, comment=""}
ODS html GPATH = ".\img\" ;
...
```{r SAS7, engine="sashtml", engine.path=saspath, include=TRUE, engine.opts=sasopts, comment=""}
ODS html GPATH = ".\img\" ;
...
### Unadjusted RCB Model

```{r SAS8, engine="sashtml", collectcode=TRUE, engine.path=saspath, include=TRUE, engine.opts=sasopts, comment=""}
...
### RCB Model with Spatial Covariates

```{r SAS9, engine="sashtml", collectcode=TRUE, engine.path=saspath, include=TRUE, engine.opts=sasopts, comment=""}
...
```{r SAS10, engine="sashtml", collectcode=TRUE, engine.path=saspath, include=TRUE, engine.opts=sasopts, comment=""}
...
In the ANOVA section (*Type 3 Tests of Fixed Effects*), we find that columns show a strong effect (p<0.0001) while rows are more moderate (p=0.0448). The AIC for this model falls between that of the unadjusted RCB design and the Spatially adjusted model. That is, it is better than no adjustment, but not as good as the spatially adjusted covariance model. It is also notable that entry has a large p-value (p=0.3583) suggesting not differences in the entries.<br><br>
...
```{r SAS11, engine="sashtml", engine.path=saspath, include=TRUE, engine.opts=sasopts, comment=""}
